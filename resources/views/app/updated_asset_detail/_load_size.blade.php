...
<div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
    <div class="w-full sm:w-1/2">
        <input type="search" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500" id="data_search" placeholder="Cari article..."/>
    </div>
...
<script>
    ...
    function loadSizeData(page = 1) {
        ...
        $.ajax({
            url: "{{ url('ad_size_datatables_simple') }}",
            type: "GET",
            data: {
                page: page,
            ...
            success: function(response) {
                if (response.error) {
                    $('#DatatbBody').html(`
                        <tr>
                            <td colspan="22" class="px-4 py-8 text-center text-red-500">
                                <div class="flex flex-col items-center">
        ...
        data.forEach(function(row) {
            html += `
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">${escapeHtml(row.p_name)}</td>
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">${escapeHtml(row.p_color)}</td>
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">${escapeHtml(row.sz_name)}</td>
    ...
    function updatePagination() {
        ...
        var prevBtn = $('<button>')
            ...
            .on('click', function() {
                if (currentPage > 1) loadSizeData(currentPage - 1);
            });
        ...
        var nextBtn = $('<button>')
            ...
            .on('click', function() {
                if (currentPage < totalPages) loadSizeData(currentPage + 1);
            });
