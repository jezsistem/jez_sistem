...
<script>
    ...
    function loadColorData(page = 1) {
        ...
        $.ajax({
            url: "{{ url('ad_color_datatables_simple') }}",
            type: "GET",
            data: {
                page: page,
            ...
            success: function(response) {
                if (response.error) {
                    $('#DatatbBody').html(`
                        <tr>
                            <td colspan="21" class="px-4 py-8 text-center text-red-500">
                                <div class="flex flex-col items-center">
                ...
                renderColorTable(response.data);
                updatePagination();
            },
            error: function(xhr) {
    ...
    function updatePagination() {
        ...
        var prevBtn = $('<button>')
            ...
            .on('click', function() {
                if (currentPage > 1) loadColorData(currentPage - 1);
            });
        ...
        var nextBtn = $('<button>')
            ...
            .on('click', function() {
                if (currentPage < totalPages) loadColorData(currentPage + 1);
            });
