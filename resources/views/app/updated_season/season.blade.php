...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage season information</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <div class="relative">
                ...
                <div id="exportMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    <div class="py-1">
                        <a href="#" id="season_excel_btn" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden p-5">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="Seasontb">
                <thead class="text-sm text-body bg-gray-100 border-b rounded-base border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 !py-3 text-left text-xs !font-medium !text-gray-500 uppercase tracking-wider !border-gray-200 text-center">Nama Season</th>
                        <th class="px-6 !py-3 text-left text-xs !font-medium !text-gray-500 uppercase tracking-wider !border-gray-200 text-center">Mulai</th>
                        <th class="px-6 !py-3 text-left text-xs !font-medium !text-gray-500 uppercase tracking-wider !border-gray-200 text-center">Selesai</th>
                    </tr>
                </thead>
...
<!-- Add/Edit Modal -->
<div id="SeasonModal" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 overflow-y-auto">
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_season">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label for="ss_name" class="block text-sm font-medium text-gray-700 mb-2">
                            Nama Season <span class="text-red-500">*</span>
                        </label>
                        <input type="text" 
                               id="ss_name" 
                               name="ss_name" 
                    ...
                    <div>
                        <label for="ss_start" class="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <div>
                        <label for="ss_end" class="block text-sm font-medium text-gray-700 mb-2">
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_import" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label for="ss_template" class="block text-sm font-medium text-gray-700 mb-2">
...
$(document).ready(function() {
    ...
    $('#ss_name').on('change', function() {
        ...
        $.ajax({
            ...
            success: function(r) {
                if (r.status == '200') {
                    showToast('Season sudah ada di sistem, silahkan ganti dengan yang lain', 'warning');
                    $('#ss_name').val('');
                }
            }
        });
    ...
    $('#f_season').on('submit', function(e) {
        ...
        $("#save_season_btn").prop("disabled", true);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#save_season_btn").prop("disabled", false);
                if (data.status == '200') {
                    closeModal();
                    loadSeasonData();
...
function loadSeasonData() {
    $.ajax({
        ...
        success: function(response) {
            ...
            if (season_dataTable) {
                season_dataTable.destroy();
            }
            
            // Clear table body
            $('#Seasontb tbody').empty();
            
            // Populate table with data
            if (response.data && response.data.length > 0) {
                response.data.forEach(function(row, index) {
                    var months = ['', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
                    var startMonth = months[parseInt(row.ss_start)] || row.ss_start;
                    var endMonth = months[parseInt(row.ss_end)] || row.ss_end;
                    ...
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" data-id="' + row.id + '" data-name="' + escapeHtml(row.ss_name || '') + '" data-start="' + (row.ss_start || '') + '" data-end="' + (row.ss_end || '') + '">' + escapeHtml(row.ss_name || '') + '</td>';
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">' + startMonth + '</td>';
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">' + endMonth + '</td>';
                    html += '</tr>';
                    $('#Seasontb tbody').append(html);
                });
            } else {
                $('#Seasontb tbody').append('<tr><td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data</td></tr>');
            }
            
            // Initialize simple-datatables
            if (document.getElementById("Seasontb") && typeof simpleDatatables !== 'undefined') {
                season_dataTable = new simpleDatatables.DataTable("#Seasontb", {
                    searchable: true,
                    sortable: true,
                    perPage: 10,
                ...
                setTimeout(function() {
                    $('#Seasontb tbody').off('click').on('click', 'tr', function() {
                        var $row = $(this);
                        var id = $row.find('td[data-id]').data('id');
                        var name = $row.find('td[data-id]').data('name');
                        var start = $row.find('td[data-id]').data('start');
                        var end = $row.find('td[data-id]').data('end');
