    ...
    $(document).ready(function() {
        ...
        var store_type_table = $('#StoreTypetb').DataTable({
            ...
            columns: [{
                ...
                {
                    data: 'stt_description',
                    name: 'stt_description'
                },
            ],
            columnDefs: [{
                "targets": 0,
        ...
        $('#delete_store_type_btn').on('click', function() {
            if (confirm("Yakin hapus data ini?")) {
                $("#delete_store_type_btn").html('Menghapus...');
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    }
                ...
                $.ajax({
                    ...
                    url: "{{ url('stt_delete') }}",
                    success: function(r) {
                        $("#delete_store_type_btn").html('Hapus');
                        if (r.status == '200') {
        ...
        $('#delete_store_type_division_btn').on('click', function() {
            if (confirm("Yakin hapus data ini?")) {
                $("#delete_store_type_division_btn").html('Menghapus...');
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    }
                ...
                $.ajax({
                    ...
                    url: "{{ url('dv_delete') }}",
                    success: function(r) {
                        $("#delete_store_type_division_btn").html('Hapus');
                        if (r.status == '200') {
                            toastr.success("Data berhasil dihapus", "Berhasil");
                            $('#StoreTypeDivision').modal('hide');
                            store_type_division_table.ajax.reload();
        ...
        $('#Storetb tbody').on('click', 'tr', function() {
            ...
            // Mengatur nilai form dan membuka modal
            jQuery.noConflict();
            $('#StoreModal').modal('show');
            jQuery('#stt_id').val(stt_id).trigger('change');
            jQuery('#dv_id').val(dv_id).trigger('change');
            ...
            // Menampilkan tombol hapus hanya jika pengguna adalah administrator
            @if ($data['user']->g_name == 'administrator')
                $('#delete_store_btn').show();
            @endif

            // Menambahkan notifikasi toastr
            toastr.info("Data store berhasil dimuat untuk diedit", "Informasi");
        });
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                ...
                                toastr.warning("Gagal hapus data", "Gagal");
                            }
                        },
                        error: function() {
