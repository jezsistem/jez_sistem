                ...
                <div class="row mb-4">
                    @foreach ($stats ?? [] as $stat)
                        <div class="col-lg-2 col-md-4">
                            <div
                                class="card card-custom mb-4 rounded-lg 
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <select class="form-control" id="date_filter" name="date_filter"
                                                ...
                                                <option value="this_week"
                                                    {{ request('date_filter', 'this_week') == 'this_week' ? 'selected' : '' }}>
                                                    This Week</option>
                                                <option value="past_week"
                                                    {{ request('date_filter', 'this_week') == 'past_week' ? 'selected' : '' }}>
                                                    Past Week</option>
                                                <option value="this_month"
                                                    {{ request('date_filter', 'this_week') == 'this_month' ? 'selected' : '' }}>
                                                    This Month</option>
                                                <option value="last_month"
                                                    {{ request('date_filter', 'this_week') == 'last_month' ? 'selected' : '' }}>
                                                    Past Month</option>
                                                <option value="custom"
                                                    {{ request('date_filter', 'this_week') == 'custom' ? 'selected' : '' }}>
                                                    Custom Range</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <select class="form-control" id="status" name="status">
                                                ...
                                                <option value="present"
                                                    {{ request('status') == 'present' ? 'selected' : '' }}>Present</option>
                                                <option value="absent"
                                                    {{ request('status') == 'absent' ? 'selected' : '' }}>Absent</option>
                                                <option value="late" {{ request('status') == 'late' ? 'selected' : '' }}>
                                                    Late</option>
                                                <option value="early_leave"
                                                    {{ request('status') == 'early_leave' ? 'selected' : '' }}>Early Leave
                                                </option>
                                                <option value="scan_once"
                                                    {{ request('status') == 'scan_once' ? 'selected' : '' }}>Scan Once
                                                </option>
                                                @foreach ($leaveTypes as $leaveType)
                                                    <option value="leave_{{ $leaveType->lt_code }}"
                                                        {{ request('status') == 'leave_' . $leaveType->lt_code ? 'selected' : '' }}>
                                                        {{ $leaveType->lt_name }}
                                                    </option>
                                                @endforeach
                                            </select>
                        ...
                        <div class="card card-custom gutter-b">
                            <div class="card-header flex-wrap py-3">
                                <div class="card-toolbar d-flex justify-content-between w-100">
                                    <div class="d-flex align-items-center">
                                        <input type="search" class="form-control" style="width: 300px;"
                                            id="attendance_search" placeholder="Search" />
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <!--begin::Button-->
                                        ...
                                        <!--end::Button-->
                                        <!--begin::Button-->
                                        <button type="button" class="btn btn-secondary font-weight-bolder mr-2"
                                            onclick="exportToPDF()">
                                            <span class="svg-icon svg-icon-md">
                                                <i class="ki-outline ki-file-down"></i>
                                        ...
                                        <!--end::Button-->
                                        <!--begin::Button-->
                                        <form method="POST" action="{{ route('attendance.reprocess-all') }}"
                                            ...
                                            <input type="hidden" name="end_date"
                                                value="{{ request('end_date', date('Y-m-d')) }}">
                                            <button type="submit" class="btn btn-light-primary font-weight-bolder mr-2"
                                                onclick="return confirm('Yakin ingin memproses ulang semua data absensi?')">
                                                <span class="svg-icon svg-icon-md">
                                        ...
                                        <a href="{{ route('attendance.upload') }}"
                                            class="btn btn-green font-weight-bolder mr-2">
                                            <span class="svg-icon svg-icon-md">
                            ...
                            <div class="card-body table-responsive">
                                ...
                                @php
                                    // Normalize session errors to array (MessageBag -> array)
                                    $sessionErrors = [];
                                    if (session()->has('errors')) {
                                        $errs = session('errors');
                                        if ($errs instanceof \Illuminate\Support\MessageBag) {
                                            $sessionErrors = $errs->all();
                                        } elseif (is_array($errs)) {
                                            $sessionErrors = $errs;
                                        } else {
                                            $sessionErrors = (array) $errs;
                                        }
                                    }
                                @endphp

                                @if (!empty($sessionErrors))
                                    <div class="alert alert-danger alert-dismissible" id="errorsAlert">
