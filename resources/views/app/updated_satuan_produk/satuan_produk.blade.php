...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage product unit information</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <div class="relative">
                ...
                <div id="exportMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    <div class="py-1">
                        <a href="#" id="satuan_produk_excel_btn" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        <div class="relative">
            ...
            <input type="search" 
                   ...
                   placeholder="Cari nama satuan produk, deskripsi...">
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden p-5">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="SatuanProduktb">
                <thead class="text-sm text-body bg-gray-100 border-b rounded-base border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 !py-3 text-left text-xs !font-medium !text-gray-500 uppercase tracking-wider !border-gray-200 text-center">Satuan Produk</th>
...
<!-- Add/Edit Modal -->
<div id="SatuanProdukModal" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 overflow-y-auto">
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_satuan_produk">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label for="pu_name" class="block text-sm font-medium text-gray-700 mb-2">
                            Satuan Produk <span class="text-red-500">*</span>
                        </label>
                        <input type="text" 
                               id="pu_name" 
                               name="pu_name" 
                    ...
                    <div>
                        <label for="pu_description" class="block text-sm font-medium text-gray-700 mb-2">
                            Deskripsi
                        </label>
                        <input type="text" 
                               id="pu_description" 
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_import" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label for="pu_template" class="block text-sm font-medium text-gray-700 mb-2">
...
$(document).ready(function() {
    ...
    loadSatuanProdukData();

    $('#satuan_produk_search').on('keyup', function() {
        loadSatuanProdukData();
    });

    // Check duplicate product unit name
    $('#pu_name').on('change', function() {
        ...
        $.ajax({
            ...
            success: function(r) {
                if (r.status == '200') {
                    showToast('Satuan Produk sudah ada di sistem, silahkan ganti dengan yang lain', 'warning');
                    $('#pu_name').val('');
                }
            }
        });
    ...
    // Form submission
    $('#f_satuan_produk').on('submit', function(e) {
        e.preventDefault();
        $("#save_satuan_produk_btn").html('Proses...');
        $("#save_satuan_produk_btn").prop("disabled", true);
        ...
        $.ajax({
            ...
            success: function(data) {
                $("#save_satuan_produk_btn").html('Simpan');
                $("#save_satuan_produk_btn").prop("disabled", false);
                if (data.status == '200') {
                    closeModal();
                    loadSatuanProdukData();
...
function loadSatuanProdukData() {
    $.ajax({
        url: "{{ url('product_unit_datatables') }}",
        type: 'GET',
        data: {
            search: $('#satuan_produk_search').val(),
            length: -1 // Get all data for simple-datatables
        },
        dataType: 'json',
        success: function(response) {
            // Destroy existing datatable if exists
            if (satuan_produk_dataTable) {
                satuan_produk_dataTable.destroy();
            }
            
            // Clear table body
            $('#SatuanProduktb tbody').empty();
            ...
            if (response.data && response.data.length > 0) {
                response.data.forEach(function(row, index) {
                    ...
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" data-id="' + row.id + '" data-name="' + escapeHtml(row.pu_name || '') + '" data-description="' + escapeHtml(row.pu_description || '') + '">' + escapeHtml(row.pu_name || '') + '</td>';
                    html += '<td class="px-6 py-4 text-sm text-gray-500">' + escapeHtml(row.pu_description || '') + '</td>';
                    html += '</tr>';
                    $('#SatuanProduktb tbody').append(html);
                });
            } else {
                $('#SatuanProduktb tbody').append('<tr><td colspan="3" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data</td></tr>');
            }
            
            // Initialize simple-datatables
            if (document.getElementById("SatuanProduktb") && typeof simpleDatatables !== 'undefined') {
                satuan_produk_dataTable = new simpleDatatables.DataTable("#SatuanProduktb", {
                    searchable: true,
                    sortable: true,
                    perPage: 10,
                ...
                setTimeout(function() {
                    $('#SatuanProduktb tbody').off('click').on('click', 'tr', function() {
                        var $row = $(this);
...
function openAddModal() {
    $('#_id').val('');
    $('#_mode').val('add');
    $('#f_satuan_produk')[0].reset();
    $('#delete_satuan_produk_btn').hide();
...
function openEditModal(id, name, description) {
    $('#pu_name').val(name);
    $('#pu_description').val(description || '');
    $('#_id').val(id);
    $('#_mode').val('edit');
    $('#delete_satuan_produk_btn').show();
    $('#modalTitle').text('Edit {{ $data["subtitle"] }}');
    openModal();
}

function openModal() {
    const modal = document.getElementById('SatuanProdukModal');
