<script>
    ...
    $(document).ready(function () {
        ...
        $(document).delegate('#pickup_item', 'click', function (e) {
            ...
            var pick_access = $(this).attr('data-pick_access'); // ambil nilai pick_access

            // 🔒 Validasi akses pickup
            if (pick_access == 0 || pick_access === '' || pick_access === null) {
                Swal.fire({
                    icon: 'error',
                    title: 'Akses Ditolak',
                    text: 'Anda tidak memiliki izin untuk pickup item ini. Hubungi Administrator.',
        ...
        // $('#br_id, #pc_id, #psc_id, #pssc_id, #sz_id, #gender_id, #p_name, #main_color_id').on('change',
        //     function () {
        //         stock_data_table.draw();
        //     });
        // $('#filter_list_btn').on('click', function () {
        //     $('#br_id, #pc_id, #psc_id, #pssc_id, #sz_id, #gender_id, #p_name, #main_color_id')
        //         .on('change', function () {
        //             stock_data_table.draw();
        //         });
        // });
        ...
        $('#change_display_btn').on('click', function () {
            ...
            $('#ChangeDisplayModal').on('show.bs.modal').modal('show');
            scanner_change_display.render(success, error);
        });

        $('#ChangeDisplayModal').on('hidden.bs.modal', function () {
            scanner_change_display.clear();
            modal_opened = '';
            scanner_main.render(success, error);
        });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response) {
                            ...
                            // Update product variant (combining color and size)
                            var variant = '';
                            if (response.p_color && response.sz_name) {
                                variant = response.p_color + ' / ' + response.sz_name;
                            } else if (response.p_color) {
                                variant = response.p_color;
                            } else if (response.sz_name) {
                                variant = response.sz_name;
                            } else {
                                variant = '-';
                            }
                    ...
                    error: function () {
                        ...
                        $('#product_name').text('-');
                        $('#product_variant').text('-');
                        toast('Error', 'Failed to fetch item data', 'error');
                    }
                });
            } else {
                // Reset fields if SKU is empty
                $('#product_name').text('-');
                $('#product_variant').text('-');
            }
        });
        ...
        $('#f_ganti_display').on('submit', function (event) {
            ...
            if (sku.trim() === '') {
                swal('Error', 'SKU tidak boleh kosong', 'error');
                return;
            }

            if (product_name.trim() === '-' || product_name.trim() === '') {
                swal('Error', 'Nama produk tidak ditemukan', 'error');
                return;
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.status == '200') {
                                swal('Berhasil',
                                    'Display stock data berhasil diubah',
                                    'success');
                                $('#ChangeDisplayModal').modal('hide');
                                stock_data_table.draw();
                            } else {
                                swal('Gagal', response.message ||
                                    'Gagal mengubah display stock data', 'error'
                                );
                            }
                        },
                        error: function () {
                            swal('Error',
                                'Terjadi kesalahan saat mengubah display stock data',
                                'error');
                        }
                    });
                }
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.status == '200') {
                                swal('Berhasil',
                                    'Pickup item berhasil dipindahkan ke display',
                                    'success');
                                waiting_list_table.draw();
                                pickup_list_table.draw();
                                stock_data_table.draw();
            ...
            success: function (response) {
                ...
                if (newCount < previousCount) {
                    ...
                    const audio = new Audio(
                        "{{ asset('music/lily.mp3') }}"); // Replace with your audio file path
                    audio.play();
                }
