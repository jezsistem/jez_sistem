<script>
    ...
    $(document).ready(function() {
        ...
        var web_category_table = $('#Wctb').DataTable({
            ...
            ajax: {
                url: "{{ url('web_category_datatables') }}",
                data: function(d) {
        ...
        $('#Wctb tbody').on('click', 'tr', function() {
            ...
            $('#psc_id').val(psc_id);
            if (cs_image != null) {
                $('#imagePreview').attr('src', "{{ asset('api/category_slug/100') }}/" + cs_image);
            }
            if (cs_banner != null) {
                $('#bannerPreview').attr('src', "{{ asset('api/category_slug/banner') }}/" + cs_banner);
            }
            $('#_id').val(id);
            $('#_mode').val('edit');
            $('#_image').val(cs_image);
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                toast("Berhasil", "Banner berhasil dihapus",
                                    "success");
