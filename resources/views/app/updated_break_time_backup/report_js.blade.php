...
function loadBreakTimeData(page = 1) {
    ...
    $.ajax({
        url: "{{ route('break-times-backup.datatables_simple_v2') }}",
        type: 'GET',
        data: {
            page: page,
            per_page: perPage,
...
function updateBreakTimeStats() {
    ...
    $.ajax({
        url: "{{ route('break-times-backup.stats_simple_v2') }}",
        type: 'GET',
        data: {
            start_date: startDate,
            end_date: endDate,
...
function exportToExcel() {
    ...
    fetch(url, {
        ...
        .then(blob => {
            ...
            a.href = downloadUrl;
            a.download = 'break_times_backup_report.xlsx';
...
function exportToPDF() {
    const url = new URL('{{ route("break-times-backup.export-pdf") }}');
