                                ...
                                <div class="col-md-7">
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                                        ...
                                                        <div>
                                                            <button type="button" class="btn btn-sm btn-dark"
                                                                onclick="viewAttachment('{{ $attachment->file_path }}', '{{ $attachment->original_name }}', '{{ $attachment->mime_type }}', {{ $attachment->file_size }})">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <small class="form-text text-muted">
                                            @if ($announcement->attachments->count() > 0)
                                                <strong>Note:</strong> If you upload new files, they will replace the
                                                current attachments
                                            @else
                                                Supported formats: Images (JPG, PNG, GIF), Documents (PDF, DOC, DOCX, XLS,
                                                XLSX, PPT, PPTX). Max size: 10MB per file.
                                            @endif
                                        </small>
                                ...
                                <div class="col-md-5">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="target_type" name="target_type" required>
                                            ...
                                            <option value="all"
                                                {{ $announcement->target_type == 'all' ? 'selected' : '' }}>All Users
                                            </option>
                                            <option value="division"
                                                {{ $announcement->target_type == 'division' ? 'selected' : '' }}>Specific
                                                Division</option>
                                            <option value="individual"
                                                {{ $announcement->target_type == 'individual' ? 'selected' : '' }}>Specific
                                                Users</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group" id="divisionGroup"
                                        style="display: {{ $announcement->target_type == 'division' ? 'block' : 'none' }};">
                                    ...
                                    <div class="form-group bg-light p-4 rounded border" id="usersGroup"
                                        style="display: {{ $announcement->target_type == 'individual' ? 'block' : 'none' }};">
                                        <label for="user_ids">Select Users <span class="text-danger">*</span></label>
                                        <div class="mb-3">
                                            <label for="user_division_filter" class="small text-muted">Filter by Division
                                                (Optional)</label>
                                        ...
                                        <div class="mb-3">
                                            <div class="w-100" role="group">
                                                ...
                                                <button type="button" class="btn btn-dark" id="clearAllBtn">Clear
                                                    All</button>
                                            </div>
                                        </div>
                                                ...
                                                @foreach ($users as $user)
                                                    ...
                                                    <div class="user-item" data-user-id="{{ $user->id }}"
                                                        ...
                                                        <label class="checkbox-inline mb-2">
                                                            ...
                                                            <small
                                                                class="badge badge-light ml-2">{{ $divisionName }}</small>
                                                        </label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        ...
                                        <div class="mt-2">
                                            <small class="text-muted">
                                                <span
                                                    id="selectedCount">{{ $announcement->recipients->where('recipient_type', 'user')->count() }}</span>
                                                user(s) selected
                                            </small>
                                        </div>

                                        <!-- Selected Users Tags -->
                                        <div class="mt-4" id="selectedUsersTags"
                                            style="display: {{ $announcement->recipients->where('recipient_type', 'user')->count() > 0 ? 'block' : 'none' }};">
                                                ...
                                                @foreach ($announcement->recipients->where('recipient_type', 'user') as $recipient)
                                                    ...
                                                    @if ($user)
                                                        <span class="badge badge-primary d-flex align-items-center"
                                                            ...
                                                            {{ $user->u_name }}
                                                            <i class="ki-outline ki-cross ml-1"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="status" name="status">
                                            <option value="active"
                                                {{ $announcement->status == 'active' ? 'selected' : '' }}>Active</option>
                                            <option value="inactive"
                                                {{ $announcement->status == 'inactive' ? 'selected' : '' }}>Inactive
                                            </option>
                                        </select>
                                    </div>
                                        ...
                                        <div class="checkbox-inline">
                                            <label class="checkbox">
                                                <input type="checkbox" id="publish_now" name="publish_now"
                                                    value="1" {{ !$announcement->published_at ? 'checked' : '' }}>
                                                <span></span>
                                                Publish immediately
                                            </label>
                                        </div>
                                    ...
                                    <div class="form-group" id="publishDateGroup"
                                        style="display: {{ $announcement->published_at ? 'block' : 'none' }};">
        ...
        (function() {
                ...
                if (userSelect && userSelect.multiple) {
                    ...
                    if (!existingInstruction) {
                        ...
                        instructionDiv.innerHTML =
                            '<i class="fas fa-info-circle mr-1"></i>Hold Ctrl (Cmd on Mac) to select multiple users';
            ...
            function initializeUserSelection() {
                ...
                if (userDivisionFilter) {
                    userDivisionFilter.addEventListener('change', filterUsers);
                }
                ...
                if (selectAllBtn) {
                    selectAllBtn.addEventListener('click', function() {
                        getVisibleUserCheckboxes().forEach(cb => cb.checked = true);
                        updateSelectedCount();
                    });
                }
                ...
                function getVisibleUserCheckboxes() {
                    return Array.from(document.querySelectorAll('.user-item'))
                ...
                function updateSelectedUsersTags() {
                    ...
                    checkedCheckboxes.forEach(function(checkbox) {
                        ...
                        if (userItem) {
                            ...
                            tag.className = 'badge badge-primary d-flex align-items-center mr-1 mb-1';
                            tag.dataset.userId = userId;
                            tag.innerHTML = `
                    ${userName} (${userNip})
                    <small class="text-white-50 ml-2">${divisionName}</small>
                    <i class="ki-outline ki-cross ml-2" style="cursor:pointer;" onclick="removeUser('${userId}')"></i>
                `;
            ...
            // Hapus user (dipanggil dari badge)
            window.removeUser = function(userId) {
                var checkbox = document.querySelector(`.user-item[data-user-id="${userId}"] .user-checkbox`);
                if (checkbox) {
                    checkbox.checked = false;
                    // Trigger change agar count & badge update
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                const attachmentElement = document.querySelector(
                                    `[onclick="removeAttachment(${attachmentId})"]`).closest(
                                    '.d-flex.align-items-center.justify-content-between');
                                attachmentElement.remove();

                                // Check if no more attachments
                                const currentAttachments = document.querySelectorAll(
                                    '.border.rounded.p-3.bg-light .d-flex.align-items-center.justify-content-between'
                                );
                                if (currentAttachments.length === 0) {
                                    // Hide the current attachments section
                                    const currentAttachmentsSection = document.querySelector(
                                        '.border.rounded.p-3.bg-light').closest('.form-group');
                                    currentAttachmentsSection.style.display = 'none';

                                    // Update label for new attachments
                                    const newAttachmentsLabel = document.querySelector(
                                        'label[for="attachments"]');
                                ...
                                showToastMessage('Error', 'Error removing attachment: ' + (data.message ||
                                    'Unknown error'), 'error');
                            }
                        })
                        .catch(error => {
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content" style="max-height: 85vh;">
                <div class="modal-header bg-light border-0 py-3">
                    ...
                    <button type="button" class="close border-0 bg-transparent" onclick="hideModal('attachmentModal')"
                        aria-label="Close" style="font-size: 1.5rem;">
                ...
                <div class="modal-footer bg-light border-0 py-2">
                    <button type="button" class="btn btn-white font-weight-bold px-4 mr-2"
                        onclick="hideModal('attachmentModal')">
