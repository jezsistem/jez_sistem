<script>
    ...
    $(document).delegate('#add_brand_to_list', 'click', function(e) {
        ...
        br_id.push(id);
        $('#brandPanel').append("<a class='btn btn-sm btn-primary mr-2 mb-2 brand-item' data-id='" + id + "'>" +
            br_name + "</a>");
        $('#brandList').fadeOut();
        $('#brand_input').val('');
    });
    ...
    $(document).delegate('#add_sub_category_to_list', 'click', function(e) {
        ...
        $('#subCategoryPanel').append(
            "<a class='btn btn-sm btn-primary mr-2 mb-2 sub-category-item' data-id='" + id + "'>" +
            psc_name + "</a>");
        $('#subCategoryList').fadeOut();
    ...
    $(document).ready(function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        error: function() {
                            toastr.error('Terjadi kesalahan dalam permintaan',
                                'Error'); // Use toastr for error handling
                        }
                    });
                    return false;
                }
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                toastr.success("Data berhasil diapprove",
                                    "Berhasil"); // Use toastr for success
                                scan_adjustment_table.draw(false);
                            } else {
                                toastr.error('Gagal approve data',
                                    'Gagal'); // Use toastr for error
                            }
                        },
                        error: function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                toastr.success("Data berhasil diselesaikan",
                                    "Berhasil"); // Use toastr for success
                                scan_adjustment_table.draw(false);
                            } else {
                                toastr.error('Gagal selesaikan data',
                                    'Gagal'); // Use toastr for error
                            }
                        },
                        error: function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    window.location.href =
                        "{{ url('export_start_scan_adjustment_bin') }}?id=" + id + "";
                }
            })
        });
        ...
        function addCustom(id, type) {
            if (done == '1') {
                swal('Dilarang', 'Anda tidak bisa menambah data ini, karena status SO sudah selesai',
                    'warning');
                return false;
            }
            $.ajaxSetup({
        ...
        function deleteCustom(id, type) {
            if (done == '1') {
                swal('Dilarang', 'Anda tidak bisa menghapus data ini, karena status SO sudah selesai',
                    'warning');
                return false;
            }
            $.ajaxSetup({
