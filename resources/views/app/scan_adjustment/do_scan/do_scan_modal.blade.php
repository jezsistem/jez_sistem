                ...
                @if ($data['sa_custom'] == '1')
                    @if (!empty($data['bin_custom']))
                        @foreach ($data['bin_custom'] as $row)
                            <a class="btn btn-sm btn-primary bin_custom mr-2 mb-2"
                                data-id="{{ $row->id }}">{{ $row->pl_code }}</a>
                        @endforeach
                    @endif
                @endif
            </div>
...
<!-- Modal-->
<div class="modal fade" id="NotFoundBarcode" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
            ...
            <form id="f_not_found">
                ...
                <div class="modal-body">
                    <div class="form-group mb-1 pb-1">
                        ...
                        <input type="text" class="form-control" id="not_found_barcode" name="not_found_barcode"
                            value="" readonly />
                    </div>
                    <div class="form-group mb-1 pb-1">
                        <label for="exampleTextarea btn-inventory">Tentukan Artikel Untuk Barcode Ini</label>
                        <input type="text" class="form-control" id="not_found_article" name="not_found_article"
                            value="" />
