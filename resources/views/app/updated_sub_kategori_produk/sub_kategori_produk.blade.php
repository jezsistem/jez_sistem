...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage product sub category information</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <div class="relative">
                ...
                <div id="exportMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    <div class="py-1">
                        <a href="#" id="sub_kategori_produk_excel_btn" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="search" 
                           ...
                           placeholder="Cari nama sub kategori, deskripsi...">
                </div>
            </div>
            <!-- Category Filter -->
            <div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden p-5">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="SubKategoriProduktb">
...
<!-- Add/Edit Modal -->
<div id="SubKategoriProdukModal" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 overflow-y-auto">
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_sub_kategori_produk">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label for="psc_name" class="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <div>
                        <label for="psc_description" class="block text-sm font-medium text-gray-700 mb-2">
                            Deskripsi
                        </label>
                        <input type="text" 
                               id="psc_description" 
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_import" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label for="psc_template" class="block text-sm font-medium text-gray-700 mb-2">
...
<script>
let sub_kategori_produk_dataTable;
...
$(document).ready(function() {
    ...
    loadSubKategoriProdukData();

    $('#sub_kategori_produk_search').on('keyup', function() {
        loadSubKategoriProdukData();
    });
    ...
    // Form submission
    $('#f_sub_kategori_produk').on('submit', function(e) {
        e.preventDefault();
        $("#save_sub_kategori_produk_btn").html('Proses...');
        $("#save_sub_kategori_produk_btn").prop("disabled", true);
        ...
        $.ajax({
            ...
            success: function(data) {
                $("#save_sub_kategori_produk_btn").html('Simpan');
                $("#save_sub_kategori_produk_btn").prop("disabled", false);
                if (data.status == '200') {
                    closeModal();
                    loadSubKategoriProdukData();
...
function loadSubKategoriProdukData() {
    $.ajax({
        url: "{{ url('product_sub_category_datatables') }}",
        type: 'GET',
        data: {
            search: $('#sub_kategori_produk_search').val(),
            pc_id: $('#pc_id_filter').val(),
            length: -1 // Get all data for simple-datatables
        },
        dataType: 'json',
        success: function(response) {
            // Destroy existing datatable if exists
            if (sub_kategori_produk_dataTable) {
                sub_kategori_produk_dataTable.destroy();
            }
            
            // Clear table body
            $('#SubKategoriProduktb tbody').empty();
            ...
            if (response.data && response.data.length > 0) {
                response.data.forEach(function(row, index) {
                    ...
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">' + escapeHtml(row.pc_name || '') + '</td>';
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" data-id="' + row.id + '" data-pc-id="' + (row.pc_id || '') + '" data-name="' + escapeHtml(row.psc_name || '') + '" data-description="' + escapeHtml(row.psc_description || '') + '">' + escapeHtml(row.psc_name || '') + '</td>';
                    html += '<td class="px-6 py-4 text-sm text-gray-500">' + escapeHtml(row.psc_description || '') + '</td>';
                    html += '</tr>';
                    $('#SubKategoriProduktb tbody').append(html);
                });
            } else {
                $('#SubKategoriProduktb tbody').append('<tr><td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data</td></tr>');
            }
            
            // Initialize simple-datatables
            if (document.getElementById("SubKategoriProduktb") && typeof simpleDatatables !== 'undefined') {
                sub_kategori_produk_dataTable = new simpleDatatables.DataTable("#SubKategoriProduktb", {
                    searchable: true,
                    sortable: true,
                    perPage: 10,
                ...
                setTimeout(function() {
                    $('#SubKategoriProduktb tbody').off('click').on('click', 'tr', function() {
                        var $row = $(this);
                        ...
                        if (id) {
                            openEditModal(id, pcId, name, description);
                        }
                    });
                }, 100);
            }
...
function openAddModal() {
    $('#_id').val('');
    $('#_mode').val('add');
    $('#f_sub_kategori_produk')[0].reset();
    $('#delete_sub_kategori_produk_btn').hide();
    $('#modalTitle').text('Tambah {{ $data["subtitle"] }}');
    openModal();
}

function openEditModal(id, pcId, name, description) {
    $('#pc_id').val(pcId || '');
    $('#psc_name').val(name);
    $('#psc_description').val(description || '');
    $('#_id').val(id);
    $('#_mode').val('edit');
    $('#delete_sub_kategori_produk_btn').show();
    $('#modalTitle').text('Edit {{ $data["subtitle"] }}');
    openModal();
}

function openModal() {
    const modal = document.getElementById('SubKategoriProdukModal');
...
// Delete function
function deleteSubKategoriProduk() {
