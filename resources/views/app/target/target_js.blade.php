    ...
    $(document).ready(function() {
        ...
        $('#tr_date').on('change', function() {
            ...
            $.ajax({
                ...
                success: function(r) {
                    if (r.status == '200') {
                        swal('Target',
                            'Periode target sudah ada disistem, silahkan ganti dengan yang lain',
                            'warning');
                        $('#tr_date').val('');
                        return false;
                    }
        ...
        $('#save_target_detail_btn').on('click', function() {
            ...
            for (let i = 0; i < total_row; i++) {
                ...
                arr[i] = [$('#target_amount' + (i + 1) + '').attr('data-date'), $('#target_amount' + (
                    i + 1) + '').val()]
            }
            $.ajaxSetup({
                headers: {
        ...
        $('#f_target').on('submit', function(e) {
            ...
            $.ajax({
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    $("#save_target_btn").html('Simpan');
                    $("#save_target_btn").attr("disabled", false);
                    toastr.error("Terjadi kesalahan: " + textStatus, "Error");
                }
            });
        });
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                toastr.success('Berhasil hapus data', 'Berhasil');
                                target_detail_table.ajax.reload();
                            } else {
                                toastr.error('Gagal hapus data', 'Gagal');
                            }
