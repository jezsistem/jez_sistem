...
<div class="mb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Edit Break Time</h1>
        </div>
...
<div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
    <form action="{{ route('break-times.update_v2', $breakTime->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                <label for="bt_type" class="block text-sm font-medium text-gray-700 mb-2">
                    Jenis Break <span class="text-red-500">*</span>
                </label>
                <select id="bt_type" name="bt_type" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option value="break_1" {{ $breakTime->bt_type === 'break_1' ? 'selected' : '' }}>Break 1</option>
                    <option value="break_2" {{ $breakTime->bt_type === 'break_2' ? 'selected' : '' }}>Break 2</option>
                </select>
            </div>
            <div>
