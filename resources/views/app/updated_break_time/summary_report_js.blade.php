...
function loadSummaryData(page = 1) {
    ...
    $.ajax({
        url: "{{ route('break-times.summary-report-datatables_simple_v2') }}",
        type: 'GET',
        data: {
            page: page,
            per_page: perPage,
        ...
        success: function(response) {
            if (response.error) {
                $('#summary_tbody').html('<tr><td colspan="8" class="px-3 py-4 text-center text-red-500">' + response.error + '</td></tr>');
                return;
            }
            ...
            if (response.data.length === 0) {
                ...
                response.data.forEach(function(row, index) {
                    ...
                    var staffLink = $('<a>')
                        .attr('href', "{{ url('break-times/staff_v2') }}/" + row.user_id)
                    ...
                    tr.append($('<td>').addClass('px-3 py-4 text-center').text(row.no_break_shifts || 0));
                    tr.append($('<td>').addClass('px-3 py-4 text-center').text(row.exceeded_break_time || 0));
        ...
        error: function() {
            $('#summary_tbody').html('<tr><td colspan="8" class="px-3 py-4 text-center text-red-500">Terjadi kesalahan saat memuat data</td></tr>');
        }
    });
}
...
function updateSummaryStats() {
    ...
    $.ajax({
        url: "{{ route('break-times.summary-report-stats_simple_v2') }}",
        type: 'GET',
        data: {
            start_date: startDate,
            end_date: endDate,
        ...
        success: function(response) {
            ...
            $('#stat-total-shifts').text(response.total_shifts || 0);
            $('#stat-total-breaks').text(response.total_breaks || 0);
            $('#stat-no-break-shifts').text(response.no_break_shifts || 0);
            $('#stat-exceeded-break-time').text(response.exceeded_break_time || 0);
...
function exportToExcel() {
    const url = new URL('{{ route("break-times.summary-report-export-excel") }}');
    ...
    link.download = 'break_time_summary_report_export.xlsx';
...
function exportToPDF() {
    const url = new URL('{{ route("break-times.summary-report-export-pdf") }}');
