<script>
    $(document).ready(function() {
        ...
        var imageUrl1 = "{{ asset('upload/background/bg-blue.png') }}";
        var imageUrl2 = "{{ asset('upload/background/bg-customer.png') }}";
        if ($(window).width() <= 768) {
        ...
        $('#cust_phone').on('change', function() {
            ...
            $.ajax({
                ...
                dataType: 'json',
                url: "{{ url('check_customer_rating_phone')}}",
                success: function(r) {
                    if (r.status == '200') {
        ...
        $('#cust_subdistrict_label').on('keyup', function(){ 
            ...
            if($.trim(query) != '' || $.trim(query) != null) {
                if ($.trim(query).length > 2) {
                    ...
                    jQuery.ajax({
                        url:"{{  url('autocomplete_subdistrict') }}",
                        method:"POST",
                        data:{query:query},
