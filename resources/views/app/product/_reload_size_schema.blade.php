            ...
            <tr>
                <td>
                    <label class="checkbox checkbox-outline">
                        <input type="checkbox" data-key="{{ $key + 1 }}" data-id="{{ $row->id }}"
                            id="product_size{{ $key + 1 }}" onclick="return getProductSize('{{ $key + 1 }}')" />
                        <span></span>
                        {{ $row->sz_name }} - {{ $row->psc_id }}
                    </label>
                </td>
                <td>
                    <input type="text" class="form-control psb_hidden" id="product_size_barcode{{ $key + 1 }}"
                        data-barcode="{{ $row->id }}" data-id="{{ $row->id }}"
                        onchange="return getProductSizeBarcode('{{ $key + 1 }}')" placeholder="Barcode"
                        style="display:none;" />
                </td>
                <td>
                    <input type="text" class="form-control psb_hidden"
                        id="product_size_running_code{{ $key + 1 }}" data-running-code="{{ $row->id }}"
                        data-id="{{ $row->id }}" style="display:none;" />
                </td>
                <td>
