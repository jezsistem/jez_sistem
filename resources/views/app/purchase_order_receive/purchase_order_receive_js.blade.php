...
<script>
            ...
            success: function(response) {
                ...
                if (Array.isArray(parsedResponse) && parsedResponse.length > 0) {
                    ...
                    swal({
                        ...
                        if (value === "export") {
                            exportToExcel(parsedResponse); // Call function to export data to Excel
                        }
                    });
                }
            },
    ...
    $('#f_upload_dispute_file').on('submit', function(e) {
        ...
        $('#upload_file_dispute_btn').attr('disabled', true);
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#upload_file_dispute_btn').html('Upload');
                $('#upload_file_dispute_btn').attr('disabled', false);
                $('#UploadFileDisputeModal').modal('hide');
    ...
    function updateCogs() {
        ...
        $('[id^="cogs_"]').each(function() {
            ...
            if (total_qty > 0) {
                ...
                var total_purchase_price = parseFloat(replaceComma($('#poad_purchase_price_' + id + '_' + index)
                    .val()));
    ...
    function receiveQty(id, index) {
        ...
        for (let i = 0; i < total_row; ++i) {
            total_price = total_price + parseFloat(replaceComma($('#total_purchase_price_receive' + id + '_' + i)
                .val()));
        }
    ...
    $(document).ready(function() {
        ...
        var poads_table = $('#Poadstb').DataTable({
            ...
            columns: [{
                ...
                {
                    data: 'poads_extra_discount',
                    name: 'poads_extra_discount',
                    orderable: false
                },
                {
                    data: 'poads_purchase_price_show',
                    name: 'poads_purchase_price',
                    orderable: false
                },
                {
                    data: 'poads_total_price_show',
        ...
        var PurchaseOrdersFileDispute = $('#FileDisputeTb').DataTable({
            ...
            dom: 'rt<"text-right"ip>',
            ajax: {
                url: "{{ url('po_dispute_file_datatable') }}", // URL sudah sesuai
                data: function(d) {
        ...
        $('#FileDisputeTb tbody').on('click', '.delete-file-dispute', function() {
            var id = $(this).data('id');
            ...
            $.ajax({
                ...
                success: function(r) {
                    if (r.status == '200') {
                        ...
                        $('#status_dispute').val(String(r.status_dispute)).trigger(
                            'change');
                        ...
                        if (r.po_shipping_cost > 0) {
                            $('#shipping_cost').val(r.po_shipping_cost).prop('disabled',
                                true).attr('value', r.po_shipping_cost);
                        } else {
                            $('#shipping_cost').val(r.po_shipping_cost).prop('disabled',
                                false).attr('value', r.po_shipping_cost);
                        }
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                ...
                                toastr.error('Gagal simpan revisi',
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                ...
                                poads_table.draw(false);
                                reloadArticleDetail($('#_po_id').val());
                                toastr.success('Data penerimaan berhasil dihapus',
                                    'Sukses'); // Use toastr for success
                            } else {
                                toastr.error('Gagal hapus penerimaan',
                                    'Error'); // Use toastr for error
                            }
                        },
                        error: function() {
                    ...
                    $.ajax({
                        ...
                        success: function(r) {
                            if (r.status == '200') {
                                ...
                                poads_table.draw(false);
                                reloadArticleDetail($('#_po_id').val());
                                toastr.success('Data gambar berhasil dihapus',
                                    'Sukses'); // Use toastr for success
                            } else {
                                toastr.error('Gagal hapus gambar',
                                    'Error'); // Use toastr for error
                            }
                        },
                        error: function() {
        ...
        $(document).ready(function() {
            $("#UploadDisputeFile").click(function() {
                $("#UploadFileDisputeModal").modal("show");
            });
        });
        ...
        //     }
        //     swal({
        //         text: "Yakin export laporan penerimaan di tanggal " + dt + " dengan status " +
        //             status + " .. ?",
        //         icon: "info",
        //         buttons: [
        //             'Batal',
        ...
        $('#export_btn').on('click', function() {
            ...
            let status = $('#po_status_filter').val();
            let dispute = $('#filter_dispute').val();

            let url = "{{ route('purchase_order_receive.export') }}" +
                "?date=" + date +
                "&status=" + status +
                "&dispute=" + dispute;
