        ...
        <table class="table table-hover table-checkable" id="PurchaseOrdertb">
            <thead class="bg-light text-dark">
                <tr>
                    ...
                    <th style="white-space: nowrap;" class="text-dark">Value Terima Terakhir</th>
                </tr>
            </thead>
            <tbody>
                @php
                ...
                @foreach ($data['product'] as $row)
                    <tr>
                        ...
                        <td style="white-space: nowrap;">
                            [{{ $row->br_name }}]<br />{{ $row->p_name }}<br />{{ $row->p_color }}<br />
                        ...
                        <td style="white-space: nowrap;">
                            <input type="text" style="width:65px;" value="Disc" readonly /><input type="text"
                                name="poa_discount" id="poa_discount{{ $row->poa_id }}" style="width:33px;"
                                value="{{ $row->poa_discount }}" onchange="return discount( {{ $row->poa_id }} )"
                                readonly /><br />
                            <input type="text" style="width:65px;" value="Ex. Disc" readonly /><input type="text"
                                name="poa_extra_discount" id="poa_extra_discount{{ $row->poa_id }}" style="width:33px;"
                                value="{{ $row->poa_extra_discount }}"
                                onchange="return extraDiscount( {{ $row->poa_id }} )" readonly /><br />
                            <input type="text"  style="width:65px;" value="Sub. Disc" readonly /><input
                                readonly type="text" name="poa_sub_discount"
                        ...
                        <td style="white-space: nowrap;">
                            ...
                            <input type="text" class="bg-primary text-white" style="width:52px;" value="Terima"
                                readonly />
                            <input type="text" class="bg-primary text-white" style="width:80px;" value="Current Stk"
                                readonly />
                        ...
                        @else --}}
                                        ...
                                        @php
                                            ...
                                            if ($srow->is_newest == 1) {
                                                $total_last_poads_price += $srow->poads_total_price;
                                            } else {
                                                $total_last_poads_price += 0;
                                            }
                                        @endphp
                        ...
                        <td><span
                                id="poads_total_price_{{ $row->poa_id }}">{{ number_format($total_poads_price) }}</span>
                        </td>
                        <td><span
                                id="poads_total_price_{{ $row->poa_id }}">{{ number_format($total_last_poads_price) }}</span>
                        </td>
                    </tr>
                    @php
                        $no++;
                        ...
                        $final_last_price_poads += $total_last_poads_price;
                    @endphp
                @endforeach
                <tr>
                    <td>
                    ...
                    <td>
                        <span class="float-right">
                            <input type="text" style="width:90px;" id="poads_total_price_receive"
                                value="{{ number_format($final_last_price_poads) }}" readonly />
                        </span>
                    </td>
                </tr>
            </tbody>
