                ...
                <tr>
                    ...
                    <td style="white-space: nowrap;">
                        ...
                        <input type="text" style="width:65px;" value="Ex. Disc" readonly/><input type="text"
                                                                                                 ...
                                                                                                 style="width:33px;"
                                                                                                 value="{{ $row->poa_extra_discount }}"
                                                                                                 onchange="return extraDiscount( {{ $row->poa_id }} )"/><br/>
                        <input type="text" disabled style="width:65px;" value="Sub. Disc" readonly/><input disabled
                                                                                                           type="text"
                                                                                                           name="poa_sub_discount"
                                                                                                           id="poa_sub_discount{{ $row->poa_id }}"
                                                                                                           style="width:33px;"
                    ...
                    <td style="white-space: nowrap;">
                        ...
                        <input type="number" id="poads_qty_remain_{{ $row->poa_id }}_{{ $i }}" style="width:60px;"
                               value="{{ ($srow->poad_qty-$srow->poads_qty) }}" readonly/>
                        @if ($srow->poad_qty-$srow->poads_qty == 0)
                                        <input type="text" style="width:52px;" value="Full" disabled/>
                                    @elseif($srow->qty_import != null)
                                        <input type="text" id="poads_qty_{{ $row->poa_id }}_{{ $i }}"
                                    ...
                                    @else
                                        <input type="text" id="poads_qty_{{ $row->poa_id }}_{{ $i }}"
                                               ...
                                               onchange="return receiveQty({{ $row->poa_id }}, {{ $i }} )" required/>
                                    @endif
                        <input type="text" disabled style="width:65px;" value="{{ $srow['total_pls_qty'] ?? '' }}"
                               readonly/>
                        @if ($srow->ps_price_tag == null || $srow->ps_price_tag == 0)
                                    ...
                                    @if($srow->qty_import != null || $srow->qty_import != 0)
                                        @if ($srow->ps_price_tag == null || $srow->ps_price_tag == 0)
                                            <input type="text" style="width:80px;" data-poad-id="{{ $srow->poad_id }}"
                                                   id="poad_purchase_price_{{ $row->poa_id }}_{{ $i }}"
                                                   onchange="return receivePurchasePrice( {{ $row->poa_id }}, {{ $i }} )"
                                                   value="{{ number_format($srow->qty_import * $row->p_price_tag) }}"/>
                                        @else
                                            <input type="text" style="width:80px;" data-poad-id="{{ $srow->poad_id }}"
                                                   id="poad_purchase_price_{{ $row->poa_id }}_{{ $i }}"
                                                   onchange="return receivePurchasePrice( {{ $row->poa_id }}, {{ $i }} )"
                                                   value="{{ number_format($srow->qty_import * $srow->ps_price_tag) }}"/>
                                        @endif
                                    @else
                                        <input type="text" style="width:80px;" data-poad-id="{{ $srow->poad_id }}"
                                               id="poad_purchase_price_{{ $row->poa_id }}_{{ $i }}"
                        ...
                        @if($srow->qty_import != null)
                                        <input type="text" style="width:90px;"
                                               ...
                                               value="{{ number_format($srow->qty_import *  $srow->poad_total_price) }}"
                                               readonly/> <img data-img-poads id="savePoads{{ $a }}"
                                                               onclick="return savePoads( {{ $srow->poad_id }}, {{ $row->poa_id }}, {{ $i }}, {{ $srow->pst_id }} )"
                                                               src="{{ asset('cdn/details_open.png') }}"
                                                               ondblclick="return bulkSavePoads( {{ $srow->poad_id }}, {{ $row->poa_id }}, {{ $i }}, {{ $srow->pst_id }} )"
                                                               src="{{ asset('cdn/details_open.png') }}"/>
                                    @else
                                        <input type="text" style="width:90px;"
                                               id="total_purchase_price_receive{{ $row->poa_id }}_{{ $i }}" value=""
                                               readonly/> <img data-img-poads id="savePoads{{ $a }}"
                                                               onclick="return savePoads( {{ $srow->poad_id }}, {{ $row->poa_id }}, {{ $i }}, {{ $srow->pst_id }} )"
                                                               src="{{ asset('cdn/details_open.png') }}"
                                                               ondblclick="return bulkSavePoads( {{ $srow->poad_id }}, {{ $row->poa_id }}, {{ $i }}, {{ $srow->pst_id }} )"
                                    ...
                                    @endif
                        <i class="fa fa-eye"
                           data-p_name="[{{ $row->br_name }}] {{ $row->p_name }} {{ $row->p_color }} [{{ $srow->sz_name }}]"
                           data-poad_id="{{ $srow->poad_id }}" id="receive_history"> </i>
                        <br/>
                                @php $i ++; $a++; $total_poad_price += $srow->poad_total_price; $total_poads_price += $srow->poads_total_price; @endphp
                            @endforeach
                        @endif
                    </td>
                    <td><span id="poad_total_price_{{ $row->poa_id }}">{{ number_format($total_poad_price) }}</span>
                    </td>
                    <td><span id="poads_total_price_{{ $row->poa_id }}">{{ number_format($total_poads_price) }}</span>
                    </td>
                </tr>
