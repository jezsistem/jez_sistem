        ...
        detailsTable.addEventListener('click', function (e) {
            ...
            rows.forEach((row, idx) => {
                row.querySelectorAll('input, textarea').forEach(input => {
                    input.name = input.name.replace(/details\[\d+\]/, `details[${idx}]`);
                });
            });
        ...
        try {
            ...
            window.externalAssignmentTable = $('#externalAssignmentTable').DataTable({
                ...
                ajax: {
                    ...
                    data : function (d) {
                        d.search = $('#assignment_search').val();
                ...
                columns: [
                    ...
                    { data: 'request_date', name: 'request_date', width: '10%' },
                    { data: 'u_name', name: 'u_name', width: '15%' },
                    { data: 'ud_name', name: 'ud_name', width: '12%' },
                    { data: 'ea_name', name: 'ea_name', width: '12%' },
                    { data: 'ear_locations', name: 'ear_locations', width: '12%' },
                    { data: 'ear_date_start', name: 'ear_date_start', width: '10%' },
                    { data: 'ear_date_end', name: 'ear_date_end', width: '10%' },
                    { data: 'ear_status', name: 'ear_status', width: '10%' },
    ...
    $(document).ready(function() {
        $('#externalAssignmentForm').on('submit', function(e) {
            ...
            $('#detailsTable tbody tr').each(function(index) {
                rundowns.push({
                    activity: $(this).find('input[name^="details["][name$="[activity]"]').val(),
                    date: $(this).find('input[name^="details["][name$="[date]"]').val(),
                    start_time: $(this).find('input[name^="details["][name$="[start_time]"]').val(),
                    end_time: $(this).find('input[name^="details["][name$="[end_time]"]').val(),
                    description: $(this).find('textarea[name^="details["][name$="[description]"]').val(),
                });
            });
            ...
            $('#cashDetailTable tbody tr').each(function(index) {
                cashDetails.push({
                    cash_purpose: $(this).find('input[name^="cash_details["][name$="[cash_purpose]"]').val(),
                    cash_amount: $(this).find('input[name^="cash_details["][name$="[cash_amount]"]').val(),
                });
            });
