    ...
    $(document).ready(function() {
        ...
        $('#Wttb tbody').on('click', 'tr td:not(:nth-child(4))', function () {
            ...
            if (pos_status == 'PAID') {
                swal({
                    ...
                    if (isConfirm) {
                        ...
                        $.ajax({
                            ...
                            success: function(r) {
                                if (r.status == '200'){
                                    var win = window.open('{{ $data['ecommerce_url'] }}/customer_paid/order/detail/id/data/'+id, '_blank');
                                    if (win) {
                                        win.focus();
                                    } else {
                ...
                var win = window.open('{{ url('/') }}/print_invoice/'+pos_invoice, '_blank');
                if (win) {
                    win.focus();
                } else {
                    alert('Please allow popups for this website');
