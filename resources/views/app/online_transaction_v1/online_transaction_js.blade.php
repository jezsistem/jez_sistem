<script>
    $(document).ready(function() {
        ...
        //     }],
        //     ajax: {
        //         url: "{{ url('transaksi_online_datatables_v1') }}",
        //         data: function(d) {
        ...
        var online_transaction_table = $('#OnlineTransactionb').DataTable({
            ...
            ajax: {
                url: "{{ url('transaksi_online_datatables_v1') }}", // URL endpoint
                data: function(d) {
        ...
        {{--    $.ajax({ --}}
        {{--        type: 'POST', --}}
        {{--        url: "{{ url('transaksi_online_import_v1')}}", --}}
        {{--        data: formData, --}}
        {{--        dataType: 'json', --}}
        {{--        cache:false, --}}
        {{--        contentType: false, --}}
        ...
        var detail_table = $('#Detailtb').DataTable({
            ...
            ajax: {
                url: "{{ url('transaksi_online_datatables_detail_v1') }}",
                data: function(d) {
                    d.to_id = $('#to_id').val();
                }
            },
        ...
        $(document).delegate('#print_invoice', 'click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ url('print_online_invoice_v1') }}',
                        method: 'POST',
                        data: {
                        ...
                        success: function(response) {
                            ...
                            if (response.status == 200) {
                                var printUrl = '{{ url('print_online_nota_v1') }}/' +
                                    numOrder;
        ...
        // $('#sales_online_export').on('click', function () {
            ...
            window.location.href = "{{ url('online_sales_export_v1') }}?branch=" + branch_trx + 
                                "&date=" + date + 
                                "&status=" + status_trx + 
