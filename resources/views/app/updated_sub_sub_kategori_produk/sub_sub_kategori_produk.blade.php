...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage product sub sub category information</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <div class="relative">
                ...
                <div id="exportMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    <div class="py-1">
                        <a href="#" id="sub_sub_kategori_produk_excel_btn" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="search" 
                           id="sub_sub_kategori_produk_search"
                           ...
                           placeholder="Cari nama sub sub kategori, deskripsi...">
                </div>
            </div>
            <!-- Sub Category Filter -->
            <div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden p-5">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="SubSubKategoriProduktb">
                <thead class="text-sm text-body bg-gray-100 border-b rounded-base border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 !py-3 text-left text-xs !font-medium !text-gray-500 uppercase tracking-wider !border-gray-200 text-center">Sub Kategori Produk</th>
                        <th class="px-6 !py-3 text-left text-xs !font-medium !text-gray-500 uppercase tracking-wider !border-gray-200 text-center">Sub Sub Kategori Produk</th>
...
<!-- Add/Edit Modal -->
<div id="SubSubKategoriProdukModal" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-50 overflow-y-auto">
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_sub_sub_kategori_produk">
                @csrf
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label for="psc_id" class="block text-sm font-medium text-gray-700 mb-2">
                            Sub Kategori Produk <span class="text-red-500">*</span>
                        </label>
                        <select id="psc_id" 
                                name="psc_id" 
                                required
                    ...
                    <div>
                        <label for="pssc_name" class="block text-sm font-medium text-gray-700 mb-2">
                            Sub Sub Kategori Produk <span class="text-red-500">*</span>
                        </label>
                        <input type="text" 
                               id="pssc_name" 
                               name="pssc_name" 
                    ...
                    <div>
                        <label for="pssc_weight" class="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <div>
                        <label for="pssc_description" class="block text-sm font-medium text-gray-700 mb-2">
                            Deskripsi
                        </label>
                        <input type="text" 
                               id="pssc_description" 
                ...
                <div class="flex items-center justify-end gap-3 border-t border-gray-200 px-6 py-4">
                    ...
                    <button type="button" 
                            id="delete_sub_sub_kategori_produk_btn" 
                            onclick="deleteSubSubKategoriProduk()"
                            style="display:none;"
                            class="px-4 py-2 text-sm font-medium text-white bg-red-500 rounded-lg hover:bg-red-700 transition-colors">
                        Hapus
                    </button>
                    <button type="submit" 
                            id="save_sub_sub_kategori_produk_btn"
        ...
        <div class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white shadow-xl transition-all">
            <form id="f_import" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label for="pssc_template" class="block text-sm font-medium text-gray-700 mb-2">
...
<script>
let sub_sub_kategori_produk_dataTable;
...
$(document).ready(function() {
    ...
    loadSubSubKategoriProdukData();

    $('#sub_sub_kategori_produk_search').on('keyup', function() {
        loadSubSubKategoriProdukData();
    });
    ...
    // Form submission
    $('#f_sub_sub_kategori_produk').on('submit', function(e) {
        e.preventDefault();
        $("#save_sub_sub_kategori_produk_btn").html('Proses...');
        $("#save_sub_sub_kategori_produk_btn").prop("disabled", true);
        ...
        $.ajax({
            ...
            success: function(data) {
                $("#save_sub_sub_kategori_produk_btn").html('Simpan');
                $("#save_sub_sub_kategori_produk_btn").prop("disabled", false);
                if (data.status == '200') {
                    closeModal();
                    loadSubSubKategoriProdukData();
...
function loadSubSubKategoriProdukData() {
    $.ajax({
        url: "{{ url('product_sub_sub_category_datatables') }}",
        type: 'GET',
        data: {
            search: $('#sub_sub_kategori_produk_search').val(),
        ...
        success: function(response) {
            ...
            if (sub_sub_kategori_produk_dataTable) {
                sub_sub_kategori_produk_dataTable.destroy();
            }
            
            // Clear table body
            $('#SubSubKategoriProduktb tbody').empty();
            ...
            if (response.data && response.data.length > 0) {
                response.data.forEach(function(row, index) {
                    ...
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" data-id="' + row.id + '" data-psc-id="' + (row.psc_id || '') + '" data-name="' + escapeHtml(row.pssc_name || '') + '" data-weight="' + (row.pssc_weight || '') + '" data-description="' + escapeHtml(row.pssc_description || '') + '">' + escapeHtml(row.pssc_name || '') + '</td>';
                    html += '<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">' + (row.pssc_weight || '') + '</td>';
                    html += '<td class="px-6 py-4 text-sm text-gray-500">' + escapeHtml(row.pssc_description || '') + '</td>';
                    html += '</tr>';
                    $('#SubSubKategoriProduktb tbody').append(html);
                });
            } else {
                $('#SubSubKategoriProduktb tbody').append('<tr><td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data</td></tr>');
            }
            
            // Initialize simple-datatables
            if (document.getElementById("SubSubKategoriProduktb") && typeof simpleDatatables !== 'undefined') {
                sub_sub_kategori_produk_dataTable = new simpleDatatables.DataTable("#SubSubKategoriProduktb", {
                    searchable: true,
                    sortable: true,
                    perPage: 10,
                ...
                setTimeout(function() {
                    $('#SubSubKategoriProduktb tbody').off('click').on('click', 'tr', function() {
                        var $row = $(this);
                        var id = $row.find('td[data-id]').data('id');
                        var pscId = $row.find('td[data-id]').data('psc-id');
                        ...
                        if (id) {
                            openEditModal(id, pscId, name, weight, description);
                        }
                    });
                }, 100);
            }
...
function openAddModal() {
    $('#_id').val('');
    $('#_mode').val('add');
    $('#f_sub_sub_kategori_produk')[0].reset();
    $('#delete_sub_sub_kategori_produk_btn').hide();
    $('#modalTitle').text('Tambah {{ $data["subtitle"] }}');
    openModal();
}

function openEditModal(id, pscId, name, weight, description) {
    $('#psc_id').val(pscId || '');
    $('#pssc_name').val(name);
    $('#pssc_weight').val(weight || '');
    $('#pssc_description').val(description || '');
    $('#_id').val(id);
    $('#_mode').val('edit');
    $('#delete_sub_sub_kategori_produk_btn').show();
    $('#modalTitle').text('Edit {{ $data["subtitle"] }}');
    openModal();
}

function openModal() {
    const modal = document.getElementById('SubSubKategoriProdukModal');
...
// Delete function
function deleteSubSubKategoriProduk() {
