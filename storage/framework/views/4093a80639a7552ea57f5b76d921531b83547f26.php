...
<script src="<?php echo e(asset('pos/js')); ?>/sweetalert.js"></script>
<script src="<?php echo e(asset('pos/js')); ?>/sweetalert1.js"></script>
...
<script>
	...
	function reloadRefund()
    {
        ...
        jQuery.ajax({
            type: "GET",
            dataType: 'html',
            url: "<?php echo e(url('reload_refund')); ?>",
            success: function(r) {
    ...
    function reloadCustomerByDivision(std_id)
    {
        ...
        jQuery.ajax({
            ...
            dataType: 'html',
            data: {_std_id:std_id},
            url: "<?php echo e(url('reload_customer_by_division')); ?>",
            success: function(r) {
    ...
    function reloadSubCustomerByDivision()
    {
        ...
        jQuery.ajax({
            type: "GET",
            dataType: 'html',
            url: "<?php echo e(url('reload_sub_customer_by_division')); ?>",
            success: function(r) {
    ...
    function reloadComplaint()
    {
        ...
        jQuery.ajax({
            type:'POST',
            url: "<?php echo e(url('check_complaint')); ?>",
            dataType: 'json',
            success: function(r) {
    ...
    function saveItem(row, pst_id, price, plst_id, price_item_discount)
    {
        ...
        jQuery.ajax({
            type:'POST',
            url: "<?php echo e(url('save_transaction_detail')); ?>",
            data: {
    ...
    jQuery(document).ready(function(e) {
        ...
        jQuery('#save_transaction').on('click', function(e) {
            ...
            jQuery.ajax({
                type:'POST',
                url: "<?php echo e(url('save_transaction')); ?>",
                data: {
                    voc_id:voc_id,
                    voc_pst_id:voc_pst_id,
                    voc_value:value_price_voc,
                ...
                success: function(r) {
                    ...
                    if (r.status == '200') {
                        ...
                        // if (finish == 'true') {
                            ...
                            if (cross != 'true') {
                                setTimeout(() => {
                                    var win = window.open('<?php echo e(url('/')); ?>/print_invoice/'+r.invoice, '_blank');
                                    if (win) {
                                        win.focus();
                                    } else {
        ...
        jQuery('#product_barcode_input').on('change', function() {
            ...
            var std_id = jQuery('#std_id').val();
            var type = jQuery('#std_id option:selected').text();
            <?php if(strtolower($data['user']->stt_name) == 'online'): ?>
            if (cust_id == '') {
                swal('Customer', 'silahkan pilih customer', 'warning');
            ...
            jQuery.ajax({
                ...
                dataType: 'json',
                url: "<?php echo e(url('check_barcode')); ?>",
                success: function(r) {
                    jQuery('#product_barcode_input').val('');
                    if (r.status == '200') {
                        var total_item = jQuery('#total_item_side').text();
        ...
        jQuery('#invoice_input').on('keyup', function(){
            ...
            if(jQuery.trim(query) != '' || jQuery.trim(query) != null) {
                if (jQuery.trim(query).length > 4) {
                    ...
                    jQuery.ajax({
                        url:"<?php echo e(url('autocomplete_invoice')); ?>",
                        method:"POST",
                        data:{query:query},
                        success:function(data){
                            jQuery('#itemList').fadeIn();
    ...
    jQuery('#refund_invoice_label').on('keyup', function(){
        ...
        if(jQuery.trim(query) != '' || jQuery.trim(query) != null) {
            if (jQuery.trim(query).length > 5) {
                ...
                jQuery.ajax({
                    url:"<?php echo e(url('autocomplete_refund_invoice')); ?>",
                    method:"POST",
                    data:{query:query},
                    success:function(data){
...
<?php /**PATH /home/sistem.jez.co.id/public_html/resources/views/app/pos/pos_js.blade.php ENDPATH**/ ?>