...
class ProductCategoryController extends Controller
{
    ...
    public function index() 
    {
        ...
        return view('app.product_category.product_category', compact('data'));
    }

    /**
     * Display product category management page (V2 - New Layout)
     */
    public function indexV2() 
    {
        ...
        return view('app.updated_kategori_produk.kategori_produk', compact('data'));
    }

    public function getDatatables(Request $request)
    {
        if(request()->ajax()) {
            return datatables()->of(ProductCategory::select('id', 'pc_name', 'pc_description')
            ->where('pc_delete', '!=', '1'))
            ->filter(function ($instance) use ($request) {
                if (!empty($request->get('search'))) {
                    $instance->where(function($w) use($request){
                        $search = $request->get('search');
                        $w->orWhere('pc_name', 'LIKE', "%$search%")
                        ->orWhere('pc_description', 'LIKE', "%$search%");
                    });
                }
            })
            ->addIndexColumn()
    ...
    public function storeData(Request $request)
    {
        ...
        $data = [
            'pc_name' => ltrim($request->input('pc_name')),
