...
class AccountTypeController extends Controller
{
    ...
    {
        $this->validateAccess('jenis_akun');
        $user = new User;
        $select = ['*'];
        $where = [
            'users.id' => Auth::user()->id
        ...
        $data = [
            'title' => $title,
            'subtitle' => DB::table('menu_accesses')->where('ma_slug', '=', 'jenis_akun')->first()->ma_title,
            'sidebar' => $this->sidebar(),
            'user' => $user_data,
            'segment' => request()->segment(1),
        ];
        return view('app.updated_account_type.account_type', compact('data'));
    }
    ...
    public function getDatatablesForSimple(Request $request)
    {
        try {
            ...
            $query = AccountType::select('id', 'at_name', 'at_description')
                ->where('at_delete', '!=', '1');

            if (!empty($search)) {
                $query->where(function ($w) use ($search) {
                    $w->where('at_name', 'LIKE', "%$search%")
                      ->orWhere('at_description', 'LIKE', "%$search%");
                });
            }
