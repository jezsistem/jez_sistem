...
class SettlementController extends Controller
{
        ...
        return datatables($combinedData)
            ...
            ->editColumn('pos_status', function ($collection) {
                if ($collection->pos_status === 'REFUND') {
                    ...
                    return '<button class="btn btn-sm btn-warning">DONE</button>';
                }
            })
    ...
    public function getNetsalesPerPaymentMethod(Request $request)
    {
            ...
            ->leftJoin('pos_transactions', function ($join) use ($start_date, $end_date, $st_id, $pm_id, $status_trx) {
                $join->on('pos_transactions.pm_id', '=', 'payment_methods.id')
                    ...
                    ->when($pm_id != 0, function ($query) use ($pm_id) {
                        return $query->where('pos_transactions.pm_id', $pm_id);
                    })
        ...
        $partial = DB::table('payment_methods')
            ...
            ->leftJoin('pos_transactions', function ($join) use ($start_date, $end_date, $st_id, $pm_id, $status_trx) {
                $join->on('pos_transactions.pm_id_partial', '=', 'payment_methods.id')
                    ...
                    ->when($pm_id != 0, function ($query) use ($pm_id) {
                        return $query->where('pos_transactions.pm_id_partial', $pm_id);
                    })
                    ->when($status_trx != '', function ($query) use ($status_trx) {
                        return $query->where('pos_transactions.pos_status', $status_trx);
                    });
        ...
        $merged = $main->merge($partial)->merge($online)
            ->groupBy('pm_name')
            ->map(function ($group) {
    ...
    private function getAllTransactions($start_date, $end_date, $st_id, $pm_id, $status_trx)
    {
        $main = DB::table('pos_transactions')
            ...
            ->join('pos_transaction_details', 'pos_transactions.id', '=', 'pt_id')
            ...
            ->select([
                ...
                DB::raw('SUM(CASE WHEN st_name like \'ONLINE%\' THEN pos_real_price ELSE pos_payment END) as netsales'),
                DB::raw('(CASE
            WHEN st_name like \'ONLINE%\' and pm_name is null then COALESCE(CONCAT(\'DEPOSIT \', UPPER(platform_name)), \'\')
            else pm_name end) as pm_name'),
                'sub_payment',
                'pos_status',
                'is_settle'
            ])
        ...
        $partial = DB::table('pos_transactions')
            ->join('stores', 'stores.id', '=', 'st_id')
            ->join('pos_transaction_details', 'pos_transactions.id', '=', 'pt_id')
            ->join('payment_methods', 'payment_methods.id', '=', 'pm_id_partial')
            ->select([
                DB::raw('DATE(ts_pos_transactions.created_at) as date'),
                'pos_invoice',
                'st_name',
                DB::raw('SUM(pos_td_qty) as qty'),
                DB::raw('SUM(CASE WHEN st_name like \'ONLINE%\' THEN pos_real_price ELSE pos_payment_partial END) as netsales'),
                'pm_name',
                'sub_payment',
                'pos_status',
                'is_settle'
