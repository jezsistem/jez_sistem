...
use App\Models\ExternalAssignmentRequestDetail;
use App\Models\ExternalAssignmentRequestReport;
...
//        return view('app.external_assignment_request.index', compact('data'));
    ...
    public function create()
    {
        ...
        $title = 'Create External Assignment Request';
        $user = auth()->user();
        $user_data = DB::table('users')->where('id', $user->id)->first();


        // Ambil semua jenis external assignment untuk dropdown
        $types = ExternalAssignmentType::orderBy('ea_name')->get();

        $data = [
            'title' => $title,
            'subtitle' => 'Create External Assignment Request',
            'sidebar' => $this->sidebar(),
            'user' => $user_data,
            'segment' => request()->segment(1)
        ];
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'status'  => 'success',
                'message' => 'External Assignment Request berhasil disimpan',
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([
    ...
    public function show($id)
    {
        ...
        $detail = ExternalAssignmentRequest::with([
            'requester:id,u_name,u_nip,ud_id',
            'approver:id,u_name',
            'hrChecker:id,u_name',
            'financeProcessor:id,u_name',
            'type:id,ea_name',
            'rundowns',
            'cashDetails',
        ])->findOrFail($id);
        ...
        $data = [
            'title' => 'Detail External Assignment Request',
            'sidebar' => $this->sidebar(),
            'user' => $user_data,
            'segment' => request()->segment(1),
        ];
...
//        dd();
        ...
        // STEP 1: Supervisor / Manager / Direktur
    ...
    public function storeReport(Request $request, $id)
    {
        ...
        if (in_array($ear->ear_status, ['Finance Process'])) {
    ...
    public function getDatatables(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query = ExternalAssignmentRequest::select([
                ...
                ->from('external_assignment_requests as ear')
            ...
            return datatables()->eloquent($query)
                ...
                ->editColumn('ear_date_start', function ($row) {
                    return $row->ear_date_start ? date('d/m/Y', strtotime($row->ear_date_start)) : '-';
                })
                ->editColumn('ear_date_end', function ($row) {
...
//        dd('kontol');
     ...
     */
    public function getExternalAssignmentSummaryDatatables(Request $request)
    {
        if ($request->ajax()) {
