...
class ScanAdjustmentController extends Controller
{
    ...
    public function indexV2()
    {
        ...
        $data = [
            'title' => $title,
            'subtitle' => DB::table('menu_accesses')->where('ma_slug', '=', 'scan_adjustment')->first()->ma_title,
            'sidebar' => $this->sidebar(),
            'user' => $user_data,
            'st_id' => DB::table('stores')->where('st_delete', '!=', '1')->orderByDesc('id')->pluck('st_name', 'id'),
            'segment' => 'scan_adjustment',
        ];
        return view('app.updated_scan_adjustment.scan_adjustment', compact('data'));
    }
