...
class B1g1Controller extends Controller
{
    ...
    public function indexV2()
    {
        ...
        $data = [
            'title' => $title,
            'subtitle' => DB::table('menu_accesses')->where('ma_slug', '=', 'b1g1_location')->first()->ma_title,
            'sidebar' => $this->sidebar(),
            'user' => $user_data,
            'pl_id' => ProductLocation::where('pl_delete', '!=', '1')->orderByDesc('id')->pluck('pl_code', 'id'),
            'st_id' => Store::where('st_delete', '!=', '1')->orderByDesc('id')->pluck('st_name', 'id'),
            'segment' => 'b1g1_location',
        ];
        return view('app.updated_b1g1_location.b1g1_location', compact('data'));
    }
