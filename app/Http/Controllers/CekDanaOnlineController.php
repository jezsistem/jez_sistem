            ...
            if (request()->ajax()) {
                return DataTables::of(
                    OnlineTransactions::select([
                        ...
                        'order_status',
                        'online_print',
                        DB::raw('online_transaction_details.original_price - online_transaction_details.discount_seller as ns_before_admin'),
...
//        $check_status_print = CekDanaOnline::where('order_number', $invoice)->whereNotNull('time_print')->count();
//
...
//                    'updated_at'   => date('Y-m-d H:i:s')
//                ];
//                CekDanaOnline::where('order_number', $invoice)->update($params);
//                $response['status'] = 200;
//            }
//            else {
//                $response['status'] = 400;
...
//        return $response;
        ...
        $check_status_print = CekDanaOnline::where('order_number', $invoice)
        ...
        if ($check_status_print > 0) {
            ...
            if (count($online_transactions) >= $sku_count) {
                ...
                // Update the transaction header to mark it as printed
                $params = [
                    'online_print' => true,
                    'u_print'      => Auth::user()->id,
                    'time_print'   => now(),
    ...
    {
        $get_invoice = array();
        $check = CekDanaOnline::where(['order_number' => $orderNumber])->exists();

        if ($check) {
            $trx = CekDanaOnline::where(['order_number' => $orderNumber])->first();
...
//        dd($st_id_form);
        ...
        if ($type === 'Shopee') {
            foreach ($data as $item) {
                ...
                try {
                    $get_order_number = CekDanaOnline::where('order_number', $order_number)->count();

                    if ($get_order_number == 0) {
                        $processedData[] = $rowData;
                        if ($rowData['order_status'] != 'Cancel' && $rowData['order_status'] != 'Batal') {
                            $transaction =  CekDanaOnline::create($rowData);

                            CekDanaOnline::where('id', $transaction->id)->update(['st_id' => Auth::user()->st_id]);
                        }
                    } else {
                        $rowUpdate = [
                            'order_number' => $order_number,
                        ...
                        $id_trx = CekDanaOnline::select('id', 'order_number', 'time_print')
                            ->where('order_number', $order_number)
                            ->first();

                        if($id_trx->time_print == NULL){
                            CekDanaOnline::where('id', $id_trx->id)->update($rowUpdate);
                            $insert_id = $id_trx->id;
                        }
                    }
                } catch (\Exception $e) {
            ...
            foreach ($data as $item) {
                ...
                try {
                    $to_id = CekDanaOnline::where('order_number', $order_number)->get()->first();
