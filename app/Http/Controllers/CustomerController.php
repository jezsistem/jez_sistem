            ...
            return datatables()->of(Customer::selectRaw("ts_customers.id as cid, ct_name, ct_id,
             ...
             ts_customers.created_at as cust_created, count(ts_pos_transactions.id) as cust_shopping")
                ...
                ->editColumn('total_pembelian', function ($data) {
                    $total = DB::table('pos_transactions')->select(DB::raw('SUM(pos_payment) as total_payment'))->where('cust_id', $data->cid)->get()->first()->total_payment;
                    $total_partials = DB::table('pos_transactions')->select(DB::raw('SUM(pos_payment_partial) as total_partial'))->where('cust_id', $data->cid)->get()->first()->total_partial;
                    $total_trx = $total + $total_partials;
                    return number_format($total_trx, 0, ',', '.');
                })
