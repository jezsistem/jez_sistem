...
class InvoiceReportController extends Controller
{
        ...
        if (request()->ajax()) {
            return datatables()->of(PosTransaction::select(
                ...
                ->editColumn('cust_name', function ($data) {
                    return '<a class="text-primary" id="customer-detail" style="white-space: nowrap; cursor:pointer;" data-id="' . $data->cust_id . '">' . $data->cust_name . '</a>';
                })
    ...
    public function detail(Request $request)
    {
        ...
        $customer = DB::table('customers')
            ->select('st_name', 'cust_name', 'cust_phone')
            ->leftJoin('stores', 'stores.id', '=', 'customers.st_id')
