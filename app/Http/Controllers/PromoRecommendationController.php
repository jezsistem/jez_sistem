...
class PromoRecommendationController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(PromoRecommendation::select('promo_recommendations.id as a_id', 'products.article_id', 'products.p_name', 'channel','products.p_price_tag', 'discount', 'notes')
                ->join('products', 'products.id', '=', 'promo_recommendations.p_id'))
