...
class ExceptionLocationController extends Controller
{
    ...
    public function indexV2()
    {
        ...
        $data = [
            'title' => $title,
            'subtitle' => DB::table('menu_accesses')->where('ma_slug', '=', 'exception_location')->first()->ma_title,
            'sidebar' => $this->sidebar(),
            'user' => $user_data,
            'pl_id' => ProductLocation::where('pl_delete', '!=', '1')->orderByDesc('id')->pluck('pl_code', 'id'),
            'segment' => 'exception_location',
        ];
        return view('app.updated_exception_location.exception_location', compact('data'));
    }
