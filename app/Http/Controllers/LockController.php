...
class LockController extends Controller
{
        ...
        public function acquireLock(Request $request)
        {
            ...
            // Hapus dulu semua lock yang sudah kedaluwarsa untuk data ini
            $lockable->locks()->where('expires_at', '<', now())->delete();
    
            // Cek lagi apakah ada lock yang masih aktif
            $existingLock = $lockable->locks()->first();
    
            if ($existingLock) {
                // Jika dikunci oleh diri sendiri, perpanjang saja waktunya
                ...
                // Jika dikunci orang lain, tolak
                return response()->json([
                    'status' => 'locked',
                    'message' => 'Data ini sedang diakses oleh pengguna lain.',
                    'locked_by' => $existingLock->user->u_name, // Beri tahu siapa yang mengunci
                ], 423); // HTTP Status 423 artinya "Locked"
            }
    
            // Jika tidak ada lock, buat lock baru
        ...
        public function releaseLock(Request $request)
        {
            ...
            $lockable = $this->resolveLockable($validated['lockable_type'], $validated['lockable_id']);
            if ($lockable) {
                // Hanya hapus lock milik user ini
                $lockable->locks()->where('user_id', Auth::id())->delete();
            }
    
            return response()->json(['status' => 'success', 'message' => 'Lock dilepaskan.']);
        }
    
        // Anda bisa menambahkan metode extendLock di sini dengan logika yang mirip
        public function extendLock(Request $request)
        {
            ...
            // Cek apakah ada lock yang aktif milik user ini
            $existingLock = $lockable->locks()->where('user_id', Auth::id())->first();
            ...
            return response()->json(['status' => 'error', 'message' => 'Tidak ada lock yang aktif untuk data ini.'], 404);
        }
}
