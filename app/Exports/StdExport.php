...
class StdExport implements FromCollection , withHeadings
{
    ...
    */
    ...
    {
        return ["Tanggal Dibuat", "Tanggal Terima", "Kode", "Pengirim", "Store Asal", "Store Tujuan", "Bin Asal","Penerima", "Brand", "SKU","Artikel", "Warna", "Size", "Qty Request", "Qty Terima", "HPP", "Harga Jual", "Status Transfer"];
    }
    ...
    public function collection()
    {
        ...
        $data = DB::table('stock_transfer_details')
                    ->selectRaw("ts_stock_transfers.created_at as created_at, ts_stock_transfer_details.pl_id,ts_stock_transfer_details.pst_id as pst_id, ts_stock_transfer_detail_statuses.created_at as created_at_receive, stf_code, ts_stock_transfers.u_id, st_id_start, st_id_end, u_id_receive, br_name, p_name, p_color, sz_name, stfd_qty, sum(ts_stock_transfer_detail_statuses.stfds_qty) as qty_receive, ps_barcode, ps_purchase_price")
        ...
        if (!empty($data)) {
            foreach ($data as $row) {
                ...
                $export[] = [date('d/m/Y H:i:s', strtotime($row->created_at)), $date_receive, $row->stf_code, $sender, $store_sender, $store_receiver, $receiver, $row->pl_id,$row->br_name, $row->ps_barcode ,$row->p_name, $row->p_color, $row->sz_name, $row->stfd_qty, $row->qty_receive, $row->ps_purchase_price, $hj, $status_transfer];
            }
        }
