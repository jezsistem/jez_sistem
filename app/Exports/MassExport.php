...
class MassExport implements FromCollection , withHeadings
{
    ...
    */
    ...
    {
        return ["Identify", "BIN", "SKU", "BRAND", "ITEM NAME", "VARIANT", "SUB KATEGORI", "Harga Beli", "Harga Jual", "QTY SYSTEM", "QTY SO"];
    }
    ...
    public function collection()
    {
        ...
        $exception = ExceptionLocation::select('pl_code')
            ...
            ->get()
            ->toArray();
        $data = DB::table('product_location_setups')->selectRaw("ts_product_location_setups.id as id, pl_code, ps_barcode, br_name, p_name, p_color, sz_name, psc_name, article_id,
        ...
        if (!empty($data->first())) {
            foreach ($data as $row) {
                ...
                $export[] = [$row->id, $row->pl_code, $row->ps_barcode, $row->br_name, $row->article_id . ' ' .$row->p_name . ' - ' . $row->p_color , $row->sz_name, $row->psc_name, $row->ps_purchase_price, $row->ps_sell_price, $stock, ""];
            }
        }
