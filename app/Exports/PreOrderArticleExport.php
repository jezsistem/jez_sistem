...
class PreOrderArticleExport implements FromCollection, WithHeadings
{
    /**
    ...
    */
    ...
    {
        $check = PreOrder::where('id', $this->po_id)->first();

        if ($check) {
            // get Purchase Order Article
            $poa_data = PreOrderArticle::select(
                ...
                'pre_order_article_details.poad_purchase_price as poad_purchase_price',
                'pre_order_article_details.poad_total_price as poad_total_price',
                'sizes.sz_schema as sz_schema',
                'pre_order_article_details.pst_id as pst_id',
                'product_location_setups.pls_qty as pls_qty'
            )
                ->leftJoin('products', 'products.id', '=', 'pre_order_articles.pr_id')
                ->leftJoin('brands', 'brands.id', '=', 'products.br_id')
                ->leftJoin('pre_order_article_details', 'pre_order_article_details.poa_id', '=', 'pre_order_articles.id')
